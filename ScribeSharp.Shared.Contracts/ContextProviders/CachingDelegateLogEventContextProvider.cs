...
{
	/// <summary>
	/// A generic context provider that uses a delegate to provide the property value. Once the delegate returns a non-null value, that value is used for all future log events and the delegate is no longer called.
	/// </summary>
	public sealed class CachingDelegateLogEventContextProvider : ContextProviderBase
	{
		...
		public CachingDelegateLogEventContextProvider(string propertyName, Func<object> requestContextValueCallback) : this(propertyName, requestContextValueCallback, null)
		{
		}
		...
		/// <exception cref="System.ArgumentNullException">Thrown if <paramref name="propertyName"/> or <paramref name="requestContextValueCallback"/> is null.</exception>
		/// <exception cref="System.ArgumentException">Thrown if <paramref name="propertyName"/> is empty or only whitespace.</exception>
		public CachingDelegateLogEventContextProvider(string propertyName, Func<object> requestContextValueCallback, ILogEventFilter filter) : base(filter)
		{
		...
		{
			if (_Value == null)
				_Value = _RequestContextValueCallback();

			if (_Value != null)
				AddProperty(logEvent.Properties, _PropertyName, _Value, rendererMap);
		}
	}
}